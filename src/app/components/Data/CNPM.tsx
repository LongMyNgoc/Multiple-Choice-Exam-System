...
import { quizCNPM } from "@/app/lib/data/CNPM";
...
const CNPM = () => {
  ...
  const getScore = () => {
    return quizCNPM.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Công nghệ Phần Mềm</h2>

      {quizCNPM.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCNPM.length} câu.
        </div>
      )}
    </div>
  );
