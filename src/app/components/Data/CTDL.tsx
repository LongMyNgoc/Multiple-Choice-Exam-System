...
import { quizCTDL } from "@/app/lib/data/CTDL";
...
const CTDL = () => {
  ...
  const getScore = () => {
    return quizCTDL.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Cấu Trúc Dữ Liệu</h2>

      {quizCTDL.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCTDL.length} câu.
        </div>
      )}
    </div>
  );
