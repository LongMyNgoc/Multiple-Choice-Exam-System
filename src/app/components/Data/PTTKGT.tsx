...
import { quizPTTKGT } from "@/app/lib/data/PTTKGT";
...
const PTTKGT = () => {
  ...
  const getScore = () => {
    return quizPTTKGT.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Phân Tích Thiết Kế Giải Thuật</h2>

      {quizPTTKGT.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizPTTKGT.length} câu.
        </div>
      )}
    </div>
  );
