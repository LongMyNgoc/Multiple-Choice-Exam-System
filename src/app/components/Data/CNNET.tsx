...
import { quizCNNET } from "@/app/lib/data/CNNET";
...
const CNNET = () => {
  ...
  const getScore = () => {
    return quizCNNET.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Công nghệ NET</h2>

      {quizCNNET.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCNNET.length} câu.
        </div>
      )}
    </div>
  );
