...
import { quizLTDT } from "@/app/lib/data/LTDT";
...
const LTDT = () => {
  ...
  const getScore = () => {
    return quizLTDT.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lý Thuyết Đồ Thị</h2>

      {quizLTDT.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizLTDT.length} câu.
        </div>
      )}
    </div>
  );
