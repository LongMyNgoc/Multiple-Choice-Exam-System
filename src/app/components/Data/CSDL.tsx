...
import { quizCSDL } from "@/app/lib/data/CSDL";
...
const CSDL = () => {
  ...
  const getScore = () => {
    return quizCSDL.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Cơ sở dữ liệu</h2>

      {quizCSDL.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCSDL.length} câu.
        </div>
      )}
    </div>
  );
