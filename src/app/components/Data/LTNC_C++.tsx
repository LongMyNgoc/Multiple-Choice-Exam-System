...
const LTNC_Cplusplus = () => {
  ...
  const getScore = () => {
    return quizLTNCC.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lập trình C++ nâng cao</h2>

      {quizLTNCC.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizLTNCC.length} câu.
        </div>
      )}
    </div>
  );
