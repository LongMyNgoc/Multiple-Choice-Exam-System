...
import { quizDTDM } from "@/app/lib/data/DTDM";
...
const DTDM = () => {
  ...
  const getScore = () => {
    return quizDTDM.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Điện Toán Đám Mây</h2>

      {quizDTDM.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizDTDM.length} câu.
        </div>
      )}
    </div>
  );
