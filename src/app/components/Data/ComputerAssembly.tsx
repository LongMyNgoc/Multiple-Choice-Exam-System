...
import { quizComputerAssembly } from "@/app/lib/data/ComputerAssembly";
import { QuizQuestion } from "@/app/types/quiz";

const optionLabels = ["A", "B", "C", "D"];

const ComputerAssembly = () => {
  ...
  const getScore = () => {
    return quizComputerAssembly.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lắp ráp, cài đặt máy tính</h2>

      {quizComputerAssembly.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizComputerAssembly.length} câu.
        </div>
      )}
    </div>
  );
