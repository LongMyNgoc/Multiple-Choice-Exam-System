...
import { quizCNJava } from "@/app/lib/data/CNJava";
...
const CNJava = () => {
  ...
  const getScore = () => {
    return quizCNJava.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Công nghệ Java</h2>

      {quizCNJava.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCNJava.length} câu.
        </div>
      )}
    </div>
  );
