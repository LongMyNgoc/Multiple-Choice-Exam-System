...
import { quizOOPC } from "@/app/lib/data/OOP_C";
...
const OOP_Cplusplus = () => {
  ...
  const getScore = () => {
    return quizOOPC.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lập trình C++ hướng đối tượng</h2>

      {quizOOPC.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizOOPC.length} câu.
        </div>
      )}
    </div>
  );
