...
import { quizLTWindow } from "@/app/lib/data/LTWindow";
...
const LTWindow = () => {
  ...
  const getScore = () => {
    return quizLTWindow.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lập Trình Window</h2>

      {quizLTWindow.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizLTWindow.length} câu.
        </div>
      )}
    </div>
  );
