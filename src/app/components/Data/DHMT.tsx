...
import { quizDHMT } from "@/app/lib/data/DHMT";
...
const DHMT = () => {
  ...
  const getScore = () => {
    return quizDHMT.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Đồ Họa Máy Tính</h2>

      {quizDHMT.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizDHMT.length} câu.
        </div>
      )}
    </div>
  );
