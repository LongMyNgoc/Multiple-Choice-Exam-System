...
const LTPHP = () => {
  ...
  const getScore = () => {
    return quizLTPHP.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Lập Trình PHP</h2>

      {quizLTPHP.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizLTPHP.length} câu.
        </div>
      )}
    </div>
  );
