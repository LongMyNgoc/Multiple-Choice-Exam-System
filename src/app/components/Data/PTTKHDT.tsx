...
import { quizPTTKHDT } from "@/app/lib/data/PTTKHDT";
...
const PTTKHDT = () => {
  ...
  const getScore = () => {
    return quizPTTKHDT.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Phân Tích Thiết Kế Hướng Đối Tượng</h2>

      {quizPTTKHDT.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizPTTKHDT.length} câu.
        </div>
      )}
    </div>
  );
