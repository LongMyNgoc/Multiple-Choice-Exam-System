...
import { quizTKWeb } from "@/app/lib/data/TKWeb";
...
const TKWeb = () => {
  ...
  const getScore = () => {
    return quizTKWeb.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Thiết Kế Web</h2>

      {quizTKWeb.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizTKWeb.length} câu.
        </div>
      )}
    </div>
  );
