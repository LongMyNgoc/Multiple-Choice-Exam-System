...
import { quizCNWeb } from "@/app/lib/data/CNWeb";
...
const CNWeb = () => {
  ...
  const getScore = () => {
    return quizCNWeb.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Công nghệ Web</h2>

      {quizCNWeb.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizCNWeb.length} câu.
        </div>
      )}
    </div>
  );
