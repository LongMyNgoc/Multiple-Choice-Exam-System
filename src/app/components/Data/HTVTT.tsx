...
import { quizHTVTT } from "@/app/lib/data/HTVTT";
...
const HTVTT = () => {
  ...
  const getScore = () => {
    return quizHTVTT.reduce((score, q) => {
  ...
  return (
    <div className="container py-5">
      <h2 className="mb-5 fw-bold text-primary">🧠 Đề thi trắc nghiệm: Hệ Tư Vấn Thông Tin</h2>

      {quizHTVTT.map((question: QuizQuestion) => (
      ...
      {!submitted ? (
        ...
        <div className="alert alert-success mt-4 fs-5 fw-bold" role="alert">
          ✅ Bạn đã làm đúng {getScore()} / {quizHTVTT.length} câu.
        </div>
      )}
    </div>
  );
